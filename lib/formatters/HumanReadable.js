...
/* eslint-enable no-underscore-dangle */
  ...
  let location = `${chunk.host}::${chunk.process.name}@${chunk.process.version}`,
      result = '';

  result += colorize(`${chunk.message} (${chunk.level})`, 'bold');
  result += '\n';
  if (chunk.process.name !== chunk.module.name) {
    location += `::${chunk.module.name}@${chunk.module.version}`;
  }
  result += colorize(location, 'white');
  if (chunk.source) {
