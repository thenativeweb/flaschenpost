...
var flaschenpost = {
  ...
  on: function (evt, callback) {
    logger.on(evt, callback);
  },

  once: function (evt, callback) {
    logger.once(evt, callback);
  },

  off: function (evt, callback) {
    logger.off(evt, callback);
  },

  pipe: function (writableStream) {
    logger.pipe(writableStream);
  },
  ...
  getLogger: function (options) {
    ...
    return {
      fatal: function (uuid, message, metadata) {
        log('fatal', uuid, message, metadata);
      },

      error: function (uuid, message, metadata) {
        log('error', uuid, message, metadata);
      },

      warn: function (uuid, message, metadata) {
        log('warn', uuid, message, metadata);
      },

      info: function (uuid, message, metadata) {
        log('info', uuid, message, metadata);
      },

      debug: function (uuid, message, metadata) {
        log('debug', uuid, message, metadata);
      }
    };
  },
